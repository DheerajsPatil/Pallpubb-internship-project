     ...
     def add(self):
         ...
         try:
             if self.var_sname.get()=="":
                 ...
                 cur.execute("Select * from school where sname=?",(self.var_sname.get(),))
                 row=cur.fetchone()
                 cur.execute("Insert into school (sname,pname,address,doj,seid,cpid,cpname,instaid,facebookid) values(?,?,?,?,?,?,?,?,?)",(
                                   self.var_sname.get(),
                                   self.var_pname.get(),
                 ...
                 con.commit()
                 messagebox.showinfo("Success","School Added Successfully",parent=self.root)
                 self.show()
         ...
         try:
             if self.var_sname.get()=="":
                 ...
                 if row==None:
                     ...
                     cur.execute("Update school set pname=?, address=?, doj=?, seid=?, cpid=?, cpname=?, instaid=?, facebookid=? where sname=?",(
                                         self.var_pname.get(),
                                         self.txt_address.get('1.0',END),
                                         self.var_date.get(),
                                         self.var_seid.get(),
          ...
          try:
               if self.var_sname.get()=="":
                 ...
                 if row==None:
                     ...
                     if op==True:
                         cur.execute("delete from school where sname=?",(self.var_sname.get(),))
                         con.commit()
     ...
     def search(self):
          ...
          try:
               if self.var_searchby.get()=="Select":
                    ...
                    cur.execute("select * from school where "+self.var_searchby.get()+" LIKE '%"+self.var_searchtxt.get()+"%'")
                    rows=cur.fetchall()
                    if len(rows)!=0:
                         self.SchoolTable.delete(*self.SchoolTable.get_children())
                         for row in rows:
