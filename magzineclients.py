...
class ClientsClass:
     def __init__(self,root):
       ...
       def openFile():
            filepath = filedialog.askopenfilename(initialdir="C:\\Users\\Asus\\OneDrive\\Desktop\\New Project\\__pycache__",
      ...
      #============Row 1=================
       lbl_custid=Label(self.root,text="Customer ID",font=("",15),bg="#324370",fg="white").place(x=300,y=250)
       ...
       lbl_language=Label(self.root,text="Language",font=("",15),bg="#324370",fg="white").place(x=850,y=300)
       
       txt_name=Entry(self.root,textvariable=self.var_name,font=("",12),bg="#FFFFFF",fg="black").place(x=450,y=300,width=230,height=25)
       txt_language=Entry(self.root,textvariable=self.var_language,font=("",12),bg="#FFFFFF",fg="black").place(x=970,y=300,width=230,height=25)

       #============Row 3=======================
       lbl_contact=Label(self.root,text="Contact",font=("",15),bg="#324370",fg="white").place(x=300,y=350)
       lbl_photo=Label(self.root,text="Photo",font=("",15),bg="#324370",fg="white").place(x=850,y=350)
       ...
       btn_photo=Button(root,text="Upload",command=openFile,font=("",12),fg="black",bd=3,relief=GROOVE).place(x=970,y=350)

       #============Row 4=========================
       lbl_address=Label(self.root,text="Address",font=("",15),bg="#324370",fg="white").place(x=300,y=400)
       btn_article=Label(self.root,text="Article",font=("",15),bg="#324370",fg="white").place(x=850,y=400)
       
       self.txt_address=Text(self.root,font=("",11),bg="white")
       self.txt_address.place(x=450,y=400, width=300, height= 60)
       btn_article=Button(root,text="Upload",command=openFile,font=("",12),fg="black",bd=3,relief=GROOVE).place(x=970,y=400)
...
#==================Clients Details====================================================
       ...
       self.ClientsTable=ttk.Treeview(clients_frame,columns=("cid","name","contact","email","address","language","photo","article"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
       ...
       self.ClientsTable.heading("language",text="LANGUAGE")
       self.ClientsTable.heading("address",text="ADDRESS")
       self.ClientsTable.heading("photo",text="PHOTO")
       self.ClientsTable.heading("article",text="ARTICLE")
       ...
       self.ClientsTable.column("language",width=100)
       self.ClientsTable.column("address",width=100)
       self.ClientsTable.column("photo",width=100)
       self.ClientsTable.column("article",width=100)
         ...
         try:
             if self.var_cust_id.get()=="":
                 ...
                 if row!=None:
                     ...
                     cur.execute("Insert into clients (cid,name,contact,email,address,language) values(?,?,?,?,?,?)",(
         ...
         try:
             if self.var_cust_id.get()=="":
                 ...
                 if row==None:
                     ...
                     cur.execute("Update clients set name=?,contact=?,email=?,address=?,language=? where cid=?",(
                                         self.var_name.get(),
                                         self.var_contact.get(),
                                         self.var_email.get(),
                                         self.txt_address.get('1.0',END),
